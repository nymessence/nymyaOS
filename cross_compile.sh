...
docker run --rm -v $(pwd):/nymyaOS --user root nymyaos-x86_64 /bin/bash -c "cd nymya_runtime && dpkg-buildpackage -us -uc -b -a amd64"
docker run --rm -v $(pwd):/nymyaOS --user root nymyaos-arm64 /bin/bash -c "cd nymya_runtime && dpkg-buildpackage -us -uc -b -a arm64"
docker run --rm -v $(pwd):/nymyaOS --user root nymyaos-riscv64 /bin/bash -c "cd nymya_runtime && dpkg-buildpackage -us -uc -b -a riscv64"