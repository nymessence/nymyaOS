...
make O="${KERNEL_OUT_DIR}" ARCH="${ARCH}" CROSS_COMPILE="${CROSS_COMPILE}" modules_prepare

# Build the kernel module
echo "🔨 Building kernel module..."
cd /nymyaOS/nymya-core
make kernel PKG_ARCH=${DEB_ARCH}
make -C "${KERNEL_OUT_DIR}" M="$(pwd)/kernel_syscalls/${DEB_ARCH}" ARCH="${ARCH}" CROSS_COMPILE="${CROSS_COMPILE}" modules
