 ...
 * CZ-SWAP gate logic using kernel-space functions, and then copies the modified
 * data back to user space. The CZ-SWAP gate is decomposed into kernel-level
 ...
 * - Error code from underlying gate operations (e.g., nymya_3311_controlled_z).
 */
SYSCALL_DEFINE2(nymya_3341_cz_swap,
    ...
    // 3. Apply the CZ-SWAP gate logic for kernel space
    // Call the kernel versions of the gates
    ret = nymya_3311_controlled_z(&k_q1, &k_q2); // Apply Controlled-Z gate
    if (ret) {
        pr_err("nymya_3341_cz_swap: Controlled-Z failed, error %d\n", ret);
        return ret;
    }

    ret = nymya_3313_swap(&k_q1, &k_q2); // Apply SWAP gate
    if (ret) {
